...
static void loadAndStoreGemmInputTile(
    ...
    bool doSplitKAcrossThreadsFirst, PatternRewriter &rewriter,
  ...
  UnitAttr splitKAcrossThreadsFirst =
      doSplitKAcrossThreadsFirst ? rewriter.getUnitAttr() : nullptr;
...
createRegInterrimBufferForAccel(PatternRewriter &rewriter, Location loc,
                                const rock::accel::AccelEmitterParams &params,
                                int64_t repeats, bool directToLDS, bool isA) {
  Value array;
  Value arrayForLoad;
  Type argType = isA ? params.argTypeA : params.argTypeB;
  ...
  if (directToLDS) {
    ...
    array = gpuAlloc(rewriter, loc, length * getByteWidth(argType),
...
struct GridwiseGemmRewritePattern : public OpRewritePattern<GridwiseGemmOp> {
  ...
  LogicalResult matchAndRewrite(GridwiseGemmOp op,
    ...
    Value start = b.createOrFold<arith::ConstantIndexOp>(loc, 0);
    scf::ForOp loopOp =
        createMainLoop(b, loc, start, nIterations, GemmLoadTileType::Default);
    {
      // inside the loop.
...
struct GridwiseAttentionAccelRewritePattern
  ...
  LogicalResult matchAndRewrite(GridwiseAttentionAccelOp op,
    ...
    RegsAsMatrixSubTiles gemm0OutSubTileViewsTrUnPadded = unpadGridSubTileView(
        rewriter, loc, gemm0OutSubTileViewsTr, prePadG0N, prePadG0M);
    ...
    auto [preAccelRegBufferKForLoad, preAccelRegBufferK] =
        createRegInterrimBufferForAccel(
            rewriter, loc, accelParamsGemm0,
            doubleBuffering ? accelParamsGemm0.mRepeats : 1, directToLDS, true);
    ...
    Value softmaxInputBuffer = createBufferForGemmOut(
        loc, elemTypeSoftmax, accelParamsGemm0, rewriter);
    SmallVector<StringRef, 3> bidGridOrder = {"g_block", "m_block", "n_block"};
    ...
    auto [preAccelRegBufferVForLoad, preAccelRegBufferV] =
        createRegInterrimBufferForAccel(
            rewriter, loc, accelParamsGemm1,
            doubleBuffering ? accelParamsGemm1.mRepeats : 1, directToLDS, true);
    ...
    if (prefetchQTile) {
      ...
      loadAndStoreGemmInputTile(
          ...
          preAccelRegBuffersQForLoad, loadTypeQ, "n", blockSize, elemTypeK,
          elemTypeKLoad, elemTypeQ, elemTypeQLoad, gemm0G, gemm0M, gemm0N,
          /*doSplitKAcrossThreadsFirst=*/false, rewriter, gemm0TuningParams,
          featuresAttr, ldsLayoutCfgNG0);
    }
    ...
    Value gemm1LDSByteBufferB = nullptr;
    if (!doBypassLDSSecondGemm)
      gemm1LDSByteBufferB = createLDSByteBuffer(
          rewriter, loc, gemm1LDSByteBufferBSize, elemTypeV);
    Value ldsReductionWorkspaceByteBuffer = nullptr;
    Value ldsReductionWorkspaceByteSecondBuffer = nullptr;
    if (op.getEnableSoftmax()) {
      ldsReductionWorkspaceByteBuffer = createLDSByteBuffer(
          rewriter, loc, reductionWorkspaceSize, elemTypeSoftmax);
      ldsReductionWorkspaceByteSecondBuffer = createLDSByteBuffer(
          rewriter, loc, reductionWorkspaceSize, elemTypeSoftmax);
    }
    ...
    layout::GridCoordinates gridCoordsGemm0 = layout::makeGxNGridLayout(
        rewriter, loc, bid, zero, gemm0NBlocks, gridSize, arch, splitKVConst);
    layout::GridCoordinates gridCoordsGemm1 = layout::makeGxNGridLayout(
        rewriter, loc, bid, zero, gemm1NBlocks, gridSize, arch, splitKVConst);

    scf::ForOp mLoopOp = createMainLoop(rewriter, loc, start, end, loadType);
    {
      ...
      auto initGemm0Stage = StageOp::create(rewriter, loc, "InitGemm0");
      {
        PatternRewriter::InsertionGuard guard(rewriter);
        rewriter.setInsertionPointToStart(
            &initGemm0Stage.getRegion().emplaceBlock());
      ...
      scf::ForOp kLoopOp =
          createMainLoop(rewriter, loc, zero, endKLoop, loadType);
      {
        PatternRewriter::InsertionGuard guard(rewriter);
        ...
        auto computeStage = StageOp::create(rewriter, loc, "MMA");
        {
          ...
          bool loadKFromLDS = loadType == GemmLoadTileType::Default ||
          ...
          BlockwiseGemmAccelOp::create(
              ...
              (ldsLayoutCfgMG0.doRotateWithK ? rewriter.getUnitAttr()
                                             : nullptr),
              (ldsLayoutCfgNG0.doRotateWithK ? rewriter.getUnitAttr()
                                             : nullptr),
              /*loadAfromLDS=*/
              (loadKFromLDS ? rewriter.getUnitAttr() : nullptr),
              /*loadBfromLDS=*/
              (loadQFromLDS ? rewriter.getUnitAttr() : nullptr),
              /*splitKAcrossThreadsFirstA=*/nullptr,
              /*splitKAcrossThreadsFirstB=*/nullptr,
              /*directToLDSA=*/(directToLDS ? rewriter.getUnitAttr() : nullptr),
              /*directToLDSB=*/
              (directToLDSQ ? rewriter.getUnitAttr() : nullptr),
              /*ldsLayoutMxK=*/
      ...
      auto postprocessGemm0Stage =
          StageOp::create(rewriter, loc, "PostProcessGemm0");
      {
        PatternRewriter::InsertionGuard guard(rewriter);
        rewriter.setInsertionPointToStart(
            &postprocessGemm0Stage.getRegion().emplaceBlock());
        ...
        // This is needed because the preSoftmaxElementWise inputs (if any),
        // don't have the GQA transformed applied to them. So, we undo the
        // transform to the output of the first GEMM. See postProcessFirstGemm()
        // to understand the transforms done to preSoftmaxElementWise inputs.
        ...
        if (op.getEnableSoftmax()) {
          ...
          Value ln2Recip = createConstantFloatOp(
              ...
              elemTypeSoftmax.getIntOrFloatBitWidth() >= 32
                  ? APFloat::opOK
                  : APFloat::opInexact);
          postProcessFirstGemmSplat<ElementwiseMultOp>(
              rewriter, loc, gridCoordsGemm0, outputFirstGemm,
              softmaxInputBuffer, gemm0OutSubTileViews,
          ...
          TypedValue<MemRefType> ldsReductionWorkspaceSecondBuffer =
              viewBufferAs(rewriter, ldsReductionWorkspaceByteSecondBuffer,
                           elemTypeSoftmax);
          BlockwiseBroadcastReduceOp::create(
              rewriter, loc, softmaxBufferExp,
              ldsReductionWorkspaceSecondBuffer, softmaxBufferSum,
              /*extraOut=*/nullptr, reductionAxis, rock::ReduceMethod::Sum,
              gemm0OutSubTileViewsTr.blockSubTile,
              gemm0OutSubTileViewsTr.blockSubTileTidSlice.value(),
              gemm0OutSubTileViewsTr.threadSubTile,
      ...
      {
        auto initGemm1Stage = StageOp::create(rewriter, loc, "InitGemm1");
        {
          PatternRewriter::InsertionGuard guard(rewriter);
          rewriter.setInsertionPointToStart(
              &initGemm1Stage.getRegion().emplaceBlock());
        ...
        scf::ForOp g1MLoopOp =
            createMainLoop(rewriter, loc, zero, endG1MLoop, loadType);
        {
          OpBuilder::InsertionGuard guard(rewriter);
          ...
          loadAndStoreGemmInputTile(
              ...
              elemTypeVLoad, elemTypeV, elemTypeVLoad, gemm0G, gemm1M, gemm1N,
              doBypassLDSSecondGemm, rewriter, gemm1TuningParams, featuresAttr,
              ldsLayoutCfgMG1);
          ...
          auto computeStage = StageOp::create(rewriter, loc, "MMA");
          {
            ...
            BlockwiseGemmAccelOp::create(
                ...
                /*loadAfromLDS=*/
                (loadVFromLDS ? rewriter.getUnitAttr() : nullptr),
                /*loadBfromLDS=*/
                !doBypassLDSSecondGemm ? rewriter.getUnitAttr() : nullptr,
                /*splitKAcrossThreadsFirstA=*/
                doBypassLDSSecondGemm ? rewriter.getUnitAttr() : nullptr,
                /*splitKAcrossThreadsFirstB=*/nullptr, /*directToLDSA=*/
                (directToLDS ? rewriter.getUnitAttr() : nullptr),
                /*directToLDSB=*/nullptr,
                /*ldsLayoutMxK=*/
                (ldsLayoutCfgMG1.ldsLayoutDxK ? rewriter.getUnitAttr()
                                              : nullptr),
    ...
    auto gridCoordsGemm1Output = layout::makeGxNGridLayout(
    ...
    ThreadwiseWriteAllOp::create(rewriter, loc, outAccBufferOutTypedFlat, trOut,
                                 outGridSubTile,
                                 /*extraIndices=*/
                                 ValueRange{gridCoordsGemm1Output.g_block,
                                            gridCoordsGemm1Output.n_block, tid},
    ...
    if (lse) {
      ...
      ThreadwiseWriteAllOp::create(rewriter, loc, lseBuffer, lse, lseMap,
                                   /*extraIndices=*/
                                   ValueRange{gridCoordsGemm1Output.g_block,
                                              gridCoordsGemm1Output.n_block,
                                              tid},
...
struct GridwiseGemmAccelRewritePattern
  ...
  LogicalResult matchAndRewrite(GridwiseGemmAccelOp op,
    ...
    auto [arrayAForLoad, arrayA] = createRegInterrimBufferForAccel(
        b, loc, params, doubleBuffering ? params.mRepeats : 1, directToLDS,
        /*isA=*/true);
    auto [arrayBForLoad, arrayB] = createRegInterrimBufferForAccel(
        b, loc, params, doubleBuffering ? params.nRepeats : 1, directToLDS,
        /*isA=*/false);
    ...
    scf::ForOp loopOp = createMainLoop(b, loc, start, nIterations, loadType);
    {
      PatternRewriter::InsertionGuard guard(b);
      b.setInsertionPointToStart(loopOp.getBody());
      Value iv = loopOp.getInductionVar();
      ...
      loadAndStoreGemmInputTile(
          ...
          elementTypeALoad, elementTypeB, elementTypeBLoad, G, M, N, false, b,
      ...
      auto stage2 = StageOp::create(b, loc, "MMA");
      {
        ...
        BlockwiseGemmAccelOp::create(
            ...
            /*splitKAcrossThreadsFirstA=*/nullptr,
            /*splitKAcrossThreadsFirstB=*/nullptr,
            /*directToLDSA=*/(directToLDS ? b.getUnitAttr() : nullptr),
            /*directToLDSB=*/(directToLDS ? b.getUnitAttr() : nullptr),
            ...
            /*bufferScaleB=*/nullptr, TypeAttr::get(elementTypeA),
