...
static gpu::AddressSpace getAddressSpace(MemrefTypedValue val) {
...
static bool isCopyRegisterOp(Operation* op) {
  ...
  for (Value operand : op->getOperands()) {
    ...
    if (auto memrefType = dyn_cast<MemRefType>(operand.getType())) {
      if (cast<gpu::AddressSpaceAttr>(memrefType.getMemorySpace()).getValue() ==
...
scheduleInstructions(OpBuilder &builder, func::FuncOp &func, scf::ForOp forOp) {
  ...
  if (llvm::any_of(copyRegistersOps, [](Operation *op) { return op == nullptr; })) {
  ...
  for (Operation *op : globalLoads)
    LLVM_DEBUG(DBGS() << *op << "\n");
  LLVM_DEBUG(DBGS() << "Found " << copyRegistersOps.size() << " copy register ops:\n");
  ...
  // if(failed(dummyMover(builder, forOp, accelOps, globalLoads, copyRegistersOps,
  //                                        ldsLoads, ldsStores))) {
  //     return failure();
  // }
